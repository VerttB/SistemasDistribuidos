...
class P2PChatClient:
    ...
    def _connect_to_peer(self, peer_info: chat_pb2.PeerInfo):
        ...
        print(f"[Sistema] Conectando ao peer '{peer_info.user_id}'...")
    ...
    def _disconnect_from_peer(self, user_id: str):
        if user_id in self.peers: print(f"[Sistema] Peer '{user_id}' saiu."); del self.peers[user_id]
    ...
    def _listen_for_discovery_events(self):
        ...
        except grpc.RpcError: print("[Sistema] Conexão com o servidor de descoberta perdida.")
        ...
        try:
            ...
            if res.history:
                ...
                for msg in res.history:
                    ...
                    # Atualiza o relógio com base no histórico para manter a consistência
    ...
    def leave_group(self):
        ...
        except grpc.RpcError as e: print(f"[Sistema] ERRO ao sair: {e.details()}")
        finally:
            self.is_listening_to_events.set()
    ...
    def send_message(self, text: str):
        # CORREÇÃO: Bloqueio movido para proteger apenas a lógica do relógio.
        with self.lock:
            self.vcm.increment()
            message = chat_pb2.ChatMessage(user_id=self.user_id, text=text, vector_clock=self.vcm.get_clock_proto(), group_id=self.group_id)
        
        # Chamadas de rede fora do bloqueio para evitar deadlock.
        # 1. Envia para os peers
        ...
        # 2. Envia para o log do servidor (fire-and-forget)
        try: self.discovery_stub.LogMessage(message)
        except grpc.RpcError: print("[Sistema] ERRO: Falha ao registrar mensagem no histórico do servidor.")
    ...
    def start_chat(self):
        ...
        try:
            while True:
                self._print_prompt(); cmd = input().strip()
                if not cmd: continue
